...
#### 방법 2: EXE 파일로 배포 (권장)
1. **빌드**: `server/build_exe.bat` 실행
2. **배포**: `dist/youtube_live_filter_server/` 폴더 전체를 원하는 위치에 복사
3. **실행**: `youtube_live_filter_server.exe` 더블클릭
4. **장점**:
   - Python 설치 불필요
   - 단일 실행 파일로 간편한 배포
   - 학습 데이터와 업데이트된 모델이 `user_data/` 폴더에 자동 저장
   - 재학습 기능 내장 (별도 스크립트 실행 불필요)

상세한 EXE 빌드 및 배포 가이드는 `server/README_EXE.md`를 참조하세요.
...
4. **모델 재학습**:
   - **EXE 사용 시**: 확장 프로그램 팝업에서 "모델 재학습" 버튼 클릭 (내장 기능)
   - **Python 직접 실행 시**: `server/train.bat` 실행
5. **모델 적용**: 학습 완료 후 확장 프로그램에서 "모델 재로드" 버튼을 클릭하여 새 모델을 적용합니다.
   - EXE 버전은 자동으로 업데이트된 모델을 사용하며, 재시작 시에도 유지됩니다.
